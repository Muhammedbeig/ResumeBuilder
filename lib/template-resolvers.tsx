...
export function resolveResumeTemplateComponent(
  ...
  const Component: ComponentType<{ data: ResumeData; className?: string }> = ({
    data,
    className,
  }) => (
    <CatalogTemplate
...
export function resolveCvTemplateComponent(
  ...
  const staticComponent =
    cvTemplateMap[templateId as keyof typeof cvTemplateMap];
  if (staticComponent) return staticComponent;
...
export function resolveCoverLetterTemplateComponent(
  ...
  const staticComponent = coverLetterTemplates.find(
    (t) => t.id === templateId,
  )?.component;
  if (staticComponent) return staticComponent;
