    ...
    >
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm text-white/70 font-light uppercase tracking-widest">
            {basics.title}
          </p>
        </div>

        <div className="space-y-4">
          <h2 className="text-sm font-bold uppercase tracking-widest text-white/50 border-b border-white/10 pb-2">
            Contact
          </h2>
          <div className="space-y-3 text-xs">
            {basics.email && (
              <div className="flex items-center gap-2">
                <Mail size={12} className="text-white/40" /> {basics.email}
              </div>
            )}
            {basics.phone && (
              <div className="flex items-center gap-2">
                <Phone size={12} className="text-white/40" /> {basics.phone}
              </div>
            )}
            {basics.location && (
              <div className="flex items-center gap-2">
                <MapPin size={12} className="text-white/40" /> {basics.location}
              </div>
            )}
            {basics.linkedin && (
              <div className="flex items-center gap-2">
                <Linkedin size={12} className="text-white/40" /> LinkedIn
              </div>
            )}
            {basics.github && (
              <div className="flex items-center gap-2">
                <Github size={12} className="text-white/40" /> GitHub
              </div>
            )}
          </div>
        </div>

        {activeStructure.map((s) =>
          sidebarTypes.includes(s.type) && s.isVisible
            ? renderSection(s)
            : null,
        )}
      </div>
      ...
      <div className="p-12">
        {activeStructure.map((s) =>
          mainTypes.includes(s.type) && s.isVisible ? renderSection(s) : null,
        )}
      </div>
    </div>
  );
