...
let cachedGroqFingerprint;
const DEFAULT_CACHE_TTL_MS = 60_000;
const WORKER_CACHE_TTL_MS = 5_000;
const DEFAULT_GROQ_MODEL = "llama-3.3-70b-versatile";
function getProviderCacheTtlMs() {
...
async function getAiSettingsFromPanel() {
    try {
        const data = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$panel$2d$internal$2d$api$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["panelInternalPost"])("settings/batch", {
            body: {
                keys: [
...
async function resolveProviderConfig() {
    ...
    const panelProvider = normalizeProvider(panelSettings?.ai_engine) || (isTruthyFlag(panelSettings?.groq_enabled) ? "groq" : null);
    const envProvider = normalizeProvider(process.env.AI_ENGINE) || (isTruthyFlag(process.env.GROQ_ENABLED) ? "groq" : null);
    let provider = panelProvider || envProvider || "gemini";
    const geminiApiKey = clean(panelSettings?.gemini_api_key) || clean(process.env.GEMINI_API_KEY);
    const groqApiKey = clean(panelSettings?.groq_api_key) || clean(process.env.GROQ_API_KEY);
    ...
    const resolved = {
        ...
        geminiApiKey,
        groqApiKey,
        groqModel: clean(panelSettings?.groq_model) || clean(process.env.GROQ_MODEL) || DEFAULT_GROQ_MODEL
    };
...
async function getGeminiClient(apiKey) {
    if (!cachedGeminiClient || cachedGeminiKey !== apiKey) {
        cachedGeminiClient = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$google$2f$generative$2d$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["GoogleGenerativeAI"](apiKey);
        cachedGeminiKey = apiKey;
    }
    return cachedGeminiClient;
}
async function getGeminiModel(model = "gemini-1.5-flash") {
    const config = await resolveProviderConfig();
    if (config.provider === "groq") {
        const fingerprint = `${config.groqApiKey}|${config.groqModel}`;
        if (!cachedGroqModel || cachedGroqFingerprint !== fingerprint) {
            cachedGroqModel = new GroqGenerativeModel(config.groqApiKey, config.groqModel);
            cachedGroqFingerprint = fingerprint;
        }
        return cachedGroqModel;
    }
    const client = await getGeminiClient(config.geminiApiKey);
...
function normalizePlanId(value) {
    ...
    if (normalized === "weekly" || normalized === "monthly" || normalized === "annual") {
        return normalized;
    }
    return null;
}
function isPaidSubscriptionValue(value) {
    const normalized = normalizeText(value);
...
function hasPaidAccess(subscription, subscriptionPlanId) {
    if (normalizePlanId(subscriptionPlanId)) {
        return true;
    }
    return isPaidSubscriptionValue(subscription);
}
function normalizeSubscriptionLevel(subscription, subscriptionPlanId) {
    if (!hasPaidAccess(subscription, subscriptionPlanId)) {
        return "free";
    }
    const normalizedSubscription = normalizeText(subscription);
    if (normalizedSubscription === "business" || normalizePlanId(subscriptionPlanId) === "annual") {
        return "business";
    }
    return "pro";
}
...
function resolveWorkerContext(request) {
    ...
    const subscriptionPlanId = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$subscription$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizePlanId"])(request.headers.get("x-rb-subscription-plan")?.trim().toLowerCase());
    const planChoice = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$plan$2d$choice$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["parsePlanChoice"])(request.headers.get("x-rb-plan-choice")?.trim().toLowerCase());
    return {
        userId,
        subscription,
        subscriptionPlanId,
...
async function requirePaidAiAccess(request) {
    ...
    const isSubscribed = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$subscription$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["hasPaidAccess"])(subscription?.subscription, subscription?.subscriptionPlanId);
...
async function requireAnnualAccess(request) {
    ...
    const isAnnual = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$subscription$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["hasPaidAccess"])(subscription.subscription, subscription.subscriptionPlanId) && (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$subscription$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizePlanId"])(subscription.subscriptionPlanId) === "annual";
    if (!isAnnual) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
"[project]/lib/worker-auth.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "getWorkerUserFromRequest",
    ()=>getWorkerUserFromRequest
]);
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$subscription$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/subscription.ts [app-route] (ecmascript)");
;
function getWorkerUserFromRequest(request) {
    const expectedKey = process.env.RB_WORKER_INTERNAL_KEY?.trim();
    if (!expectedKey) return null;
    const requestKey = request.headers.get("x-rb-worker-key")?.trim();
    if (!requestKey || requestKey !== expectedKey) return null;
...
"[project]/worker/app/api/ai/parse/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$worker$2d$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/worker-auth.ts [app-route] (ecmascript)");
;
;
;
;
...
async function POST(request) {
    const workerUser = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$worker$2d$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getWorkerUserFromRequest"])(request);
    const session = workerUser ? null : await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2d$auth$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getServerSession"])(__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$auth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["authOptions"]);
    const userId = workerUser?.id ?? session?.user?.id ?? null;
    if (!userId) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: "Unauthorized"
        }, {
    ...
    const limitedText = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$limits$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["truncateText"])(text, resourceSettings.limits.aiText);
    try {
  ...
  Extract as much information as possible. Use empty string for missing data. Return valid JSON only.`;
        ...
        let parsed = {};
        try {
            parsed = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$ai$2d$utils$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["extractJson"])(response.text());
        } catch  {
            parsed = {};
        }
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7d927faa._.js.map