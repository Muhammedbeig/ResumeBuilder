...
async function resolveUserIdFromPayPalOrder(
  orderId: string,
  paymentTransactionId: string,
) {
  const { order } = await fetchPayPalOrder(orderId);
  const purchaseUnit = order.purchase_units?.[0];
  const custom = parsePayPalCustomId(
  ...
  if (!userId) {
    throw new PayPalConfirmationError(
      "Unable to resolve user from PayPal order",
      400,
    );
  }
...
export async function POST(request: Request) {
  ...
  const paymentTransactionId = parseNumericId(
  ...
  try {
    const effectiveUserId =
      sessionUserId ||
      (await resolveUserIdFromPayPalOrder(orderId, paymentTransactionId));
    ...
    return NextResponse.json(
      { error: "Unable to confirm PayPal payment." },
      { status: 500 },
    );
  }
}
